...
<head>
	...
	<script type="text/javascript">
		...
		function page(n, s) {
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action", "${ctx}/points/exchange/list").submit();
	    	return false;
	    }
		//禁用
		function disable(actId){
	    	var url="${ctx}/points/exchange/disable?actId="+actId;
			return confirmx('确认要禁用活动吗？',url);
		}
		//启用
		function enable(actId){
			var url="${ctx}/points/exchange/enable?actId="+actId;
		...
		<div class="row-fluid">
			<div class="span12">
				<ul class="nav nav-tabs">
					<li class="active"><a href="${ctx}/points/exchange/list">活动查询</a></li>
					<li><a href="${ctx}/points/exchange/form">新增活动</a></li>
				</ul>
				<form:form id="searchForm" modelAttribute="activity"  method="post" class="breadcrumb form-search form-horizontal">
					...
					<input id="hotSale" name="hotSale" type="hidden" value="0"/>
					<div class="row-fluid">
					   <div class="span4">
					...
					<tbody>
						...
						<tr id="${item.actId}">
							<td width="15%">
								...
								&nbsp;&nbsp;
								<a href="${ctx}/points/exchange/form?actId=${item.actId}">【修改】</a>
							</td>
							<td><a href="${ctx}/points/exchange/detail?actId=${item.actId}">${item.actId}</a></td>
							<td>${item.actName}</td>
							<td>
								<c:choose>
