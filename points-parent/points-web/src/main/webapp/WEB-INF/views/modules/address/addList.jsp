...
<head>
	...
	<script type="text/javascript">
		...
		function page(n, s) {
			$("#pageNo").val(n);	
			$("#pageSize").val(s);	
			$("#searchForm").attr("action", "${ctx}/points/address").submit();
	    	return false;
	    }
	</script>
</head>
		...
		<div class="row-fluid">
			<div class="span12">
				<ul class="nav nav-tabs">
					<li class="active"><a href="${ctx}/points/address">收货地址查询</a></li>
				</ul>
				<form:form id="searchForm" modelAttribute="addressInfo"  method="post" class="breadcrumb form-search form-horizontal">
					...
					<div class="row-fluid">
					   <div class="span6">
							<div class="control-group">
								...
								<div class="controls">
									<form:input path="memMobile" htmlEscape="false" maxlength="50" class="input-large"/>
								</div>
							</div>
						</div>
						<div class="span6">
				...
				<table id="contentTable" class="table table-striped table-bordered table-condensed" style="font-size: 13px;">
					...
					<tbody>
						...
						<tr id="${item.addId}">
							...
							<c:if test="${item.isDefault eq '0'}">
							否
							</c:if>
							</td>
							<td><fmt:formatDate value="${item.createTime}" type="date" pattern="yyyy-MM-dd HH:mm:ss"/></td>
							<td><fmt:formatDate value="${item.updateTime}" type="date" pattern="yyyy-MM-dd HH:mm:ss"/></td>
						</tr>
						</c:forEach>
					</tbody>
				</table>
