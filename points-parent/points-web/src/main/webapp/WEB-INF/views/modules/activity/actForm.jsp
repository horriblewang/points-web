		...
		$(function(){
			...
			$("#btnSave").click(function(){
				if($("#inputForm").valid()){
					...
					if(total > 100){
						top.$.jBox.tip('抽奖礼品中奖概率之和必须 <= 100！','error');
						return;
					}
					var arr = new Array();
					$("#contentTable tbody tr").each(function(i){
						...
						$(this).find("td").each(function(j){
							if(j==1){
								...
								obj['percent'] = $(this).find("input:eq(0)").val();
							}else if(j == 3){
			...
			$("#buttonAddShop").click(function(){
				...
				htl += '<td><input type="text" name="percent" class="input-medium required isZhengshu"/></td>';
			...
			<div class="span12">
				  <ul class="nav nav-tabs">
					<c:if test="${not empty activity.actId}">
						<li class="active"><a href="${ctx}/points/activity/form?actId=${activity.actId}">修改活动</a></li>
					</c:if>
					<c:if test="${empty activity.actId}">
						<li class="active"><a href="${ctx}/points/activity/form">新建活动</a></li>
					</c:if>
					<c:if test="${empty activity.actId}">
						<li><a href="${ctx}/points/activity/list">活动列表</a></li>
					</c:if>
				</ul>
				<form:form id="inputForm" modelAttribute="activity" action="${ctx}/points/activity/save" method="post" class="breadcrumb form-search form-horizontal">
					...
					<div class="row-fluid">
						<div class="span6">
							<div class="control-group">
								<label class="control-label">抽奖礼品：</label>
								<div class="controls">
						...
						<table style="font-size: 13px;"id="contentTable" class="table table-striped table-bordered table-condensed">
							...
							<tbody id="content_body">
								<c:forEach var="item" items="${details}">
									...
									<td><input type="text" name="percent" value="${item.percent}" class="input-medium required isZhengshu valid"></td>
